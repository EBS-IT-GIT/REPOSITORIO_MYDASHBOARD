...
--LINEAGE          
        cast(ldg.last_update_date as timestamp) AS last_update_date,
        ldg.last_updated_by,
        cast(ldg.creation_date as timestamp) AS creation_date,
        ldg.created_by,
...
--LINEAGE 
        cast(ps.last_update_date as timestamp) AS last_update_date,
        ps.last_updated_by,
        cast(ps.creation_date as timestamp) AS creation_date,
        ps.created_by,
...
--LINEAGE          
        cast(gps.last_update_date as timestamp) AS last_update_date,
        gps.last_updated_by,
        cast(gps.creation_date as timestamp) AS creation_date,
        gps.created_by,
...
--LINEAGE 
        cast(c.last_update_date as timestamp) AS last_update_date,
        c.last_updated_by
    FROM
...
--PROPERTIES           
        ...
        fsg.display_size,
--LINEAGE     
        cast(fsg.last_update_date as timestamp) AS last_update_date,
        fsg.last_updated_by,
        cast(fsg.creation_date as timestamp) AS creation_date,
        fsg.created_by,
...
--BALANCE VALUES
        ...
        b.begin_balance_cr,
--LINEAGE 
        cast(b.last_update_date as timestamp) AS last_update_date,
        b.last_updated_by
    FROM
        gl.gl_balances b    
        ;
...
--LINEAGE        
        cast(greatest(
                ...
                us.last_update_date) as timestamp) AS last_update_date,
        fv.last_updated_by,
        cast(fv.creation_date as timestamp) AS creation_date,
        fv.created_by,
...
--LINEAGE 
        cast(greatest(
                ...
                us.last_update_date) as timestamp) AS last_update_date,                
        app.last_updated_by,
        cast(app.creation_date as timestamp) AS creation_date,        
        app.created_by,
...
--RELATIONSHIP
        ...
        u.person_party_id, 
--LINEAGE
        cast(u.last_update_date as timestamp) AS last_update_date,
        u.last_updated_by,
        cast(u.creation_date as timestamp) AS creation_date,
        u.created_by,
        u.last_update_login
    FROM
        applsys.fnd_user u
...
--PLANOS DE CONTAS
    ...
    --LINEAGE        
        cast(fst.last_update_date as timestamp) AS last_update_date,
        fst.last_updated_by,
        cast(fst.creation_date as timestamp) AS creation_date,
        fst.created_by,
...
--RELATIONSHIP        
        ...
        coalesce(stb.etb_information1, lep.le_information4) as ibge_code,
        ...
        lep.le_information2 as currency_code,
--VALIDITY
...
--LINEAGE
        cast(greatest(stb.last_update_date, 
                  lep.last_update_date, 
                  stb_reg.last_update_date, 
                  lep_reg.last_update_date, 
                  fed_jur.last_update_date,
                  coalesce(state_jur_reg.last_update_date, stb.last_update_date),
                  coalesce(munic_jur_reg.last_update_date, stb.last_update_date),
                  glev.last_update_date,
                  p.last_update_date) as timestamp) AS last_update_date,
        stb.last_updated_by,        
        cast(stb.creation_date as timestamp) AS creation_date,
        stb.created_by,
    ...
    WHERE
        ...
        AND ( ( fvs.segment_num = 6
              ...
              OR ( fvs.segment_num = 4
                   ...
                   ;
----------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW XXSTN.XXSTN_DTL_JOURNAL_BATCH AS
    SELECT
--IDENTIFICATION
...
--MAIN PROPERTIES						       
		...
		cast(b.date_created as timestamp) AS date_created,
		cast(b.posted_date as timestamp) AS posted_date,
        b.default_effective_date, 
		cast(b.earliest_postable_date as timestamp) AS earliest_postable_date,		
...
--LINEAGE        
        cast(b.last_update_date as timestamp) AS last_update_date,
        b.last_updated_by,
        cast(b.creation_date as timestamp) AS creation_date,
        b.created_by,        
        b.last_update_login
    FROM
        gl.gl_je_batches b
...
----------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW XXSTN.XXSTN_DTL_JOURNAL_HEADER AS
    SELECT 
--IDENTIFICATION
...
--MAIN PROPERTIES
        h.status,
        h.currency_code,		
		cast(h.date_created as timestamp) AS date_created,
		cast(h.posted_date as timestamp) AS posted_date,        
		h.default_effective_date,
        cast(h.earliest_postable_date as timestamp) AS earliest_postable_date,		
--DESCRIPTION      
...
--LINEAGE     
        cast(h.last_update_date as timestamp) AS last_update_date,
        h.last_updated_by,
        cast(h.creation_date as timestamp) AS creation_date,
        h.created_by,        
		h.last_update_login
    FROM
        gl.gl_je_headers h
...
----------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW XXSTN.XXSTN_DTL_JOURNAL_LINES AS
    SELECT 
--IDENTIFICATION
...
--LINEAGE 
        cast(ln.last_update_date as timestamp) AS last_update_date,
        ln.last_updated_by,
        cast(ln.creation_date as timestamp) AS creation_date,
        ln.created_by,
...
--RELATIONSHIP        
        ...
        lep_reg.location_id           AS main_location_id,    
        coalesce(lep.le_information4, stb.etb_information1) as ibge_code,        
        coalesce(lep.le_information_context, 
                 stb.etb_information_context, 
                 lep_reg.reg_information_context, 
                 stb_reg.reg_information_context) AS country_code,       
...
--DESCRIPTION
        ...
        UPPER(stb.name)                AS alternate_name,   
--LINEAGE        
        cast(greatest(lep.last_update_date,
                 stb.last_update_date,
                 stb_reg.last_update_date,
                 lep_reg.last_update_date,
                 fed_jur.last_update_date,
                 glev.last_update_date,
                 p.last_update_date) as timestamp) AS last_update_date,
        stb.last_updated_by,        
        cast(lep.creation_date as timestamp) AS creation_date,
        stb.created_by,
        stb.last_update_login
    FROM
        xle.xle_entity_profiles          lep
...
--LINEAGE          
        cast(greatest(loc.last_update_date, 
                  ter.last_update_date, 
                  ptb.last_update_date, 
                  us.last_update_date) as timestamp) AS last_update_date,
        loc.last_updated_by,        
        cast(loc.creation_date as timestamp) AS creation_date,
        loc.created_by,
...
--VALIDITY            
        ...
        o.date_to,
--LINEAGE         
        cast(greatest(o.last_update_date, 
                 o2.last_update_date, 
                 o3.last_update_date, 
                 ptb.last_update_date, 
                 us.last_update_date,
                 nvl(fin.last_update_date, o.last_update_date), 
                 nvl(ref.last_update_date, o.last_update_date)) as timestamp) AS last_update_date,
        o.last_updated_by,        
...
--LINEAGE         
        cast(pt.last_update_date as timestamp) AS last_update_date,
        pt.last_updated_by,
        cast(pt.creation_date as timestamp) AS creation_date,
        pt.created_by,
...
--LINEAGE   
        cast(fvs.last_update_date as timestamp) AS last_update_date,
        fvs.last_updated_by,
        cast(fvs.creation_date as timestamp) AS creation_date,
        fvs.created_by,

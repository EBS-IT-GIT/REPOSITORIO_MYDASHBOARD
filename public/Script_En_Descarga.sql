...
DECLARE
  ...
  l_recibido_flag VARCHAR2(1);

    
  CURSOR c_viajes IS    
     SELECT xcv.*, xtb.CARTA_PORTE_ID, xcp.TICKET_ENVIO_ID, xcp.RECIBIDO_FLAG, xest.CAMPO, (SYSDATE - NVL(fecha_maxima_retorno, SYSDATE))*24 dif_tiempo
      FROM apps.XX_TCG_CONTROL_VIAJES xcv,
                   apps.XX_TCG_TICKETS_BALANZA xtb,
                   apps.XX_TCG_CARTAS_PORTE_ALL xcp,
                   apps.XX_OPM_ESTABLECIMIENTOS xest    
      WHERE xcv.campania         = 'CA1920'
      AND xcv.viaje_id = xtb.viaje_id
      AND xtb.ticket_id = xcp.ticket_envio_id
      AND xest.ESTABLECIMIENTO_ID = xtb.ESTABLECIMIENTO_ID;
...
BEGIN
 ...
 DBMS_OUTPUT.PUT_LINE(' Inicio: ');
  ...
  FOR r_viajes IN c_viajes LOOP
      ...
      l_recibido_flag := r_viajes.recibido_flag;
      l_ticket_balanza_id := NULL;
      l_carta_porte_id := NULL;            

     IF  ((l_estado = 'EN_DESCARGA') AND (l_recibido_flag = 'Y')) THEN
      ...
      IF l_cancela THEN
        ...
        DBMS_OUTPUT.PUT_LINE('Se pasa a Descargado el viaje: '||'|'||r_viajes.viaje_id||'|'||'Turno: '||'|'||r_viajes.turno||'|'||'QR: '||'|'||r_viajes.qr_id ||
                               ' l_estado: ' || '|' || l_estado ||'Establecimiento: '||'|'||r_viajes.campo);

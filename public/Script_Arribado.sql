     ...
     SELECT xcv.*,  (SYSDATE - NVL(fecha_maxima_retorno, SYSDATE))*24 dif_tiempo
      ...
      WHERE campania         = 'CA1920'
           AND NOT EXISTS (SELECT 'Y'
                                               ...
                                               WHERE xttb.viaje_id = xcv.viaje_id)
       UNION
      SELECT xcv.*,  (SYSDATE - NVL(fecha_maxima_retorno, SYSDATE))*24 dif_tiempo
      FROM apps.XX_TCG_CONTROL_VIAJES xcv       
      WHERE campania         = 'CA1920'
           AND  EXISTS (SELECT 'Y'
                                        FROM apps.XX_TCG_TICKETS_BALANZA xttb
                                      WHERE xttb.viaje_id = xcv.viaje_id
                                           AND xttb.carta_porte_id IS NULL) 
       ;
...
BEGIN
 ...
 DBMS_OUTPUT.PUT_LINE(' Inicio: ');
  ...
  FOR r_viajes IN c_viajes LOOP
      ...
      IF l_estado = 'ARRIBADO' THEN
                       
         l_cancela := TRUE;
        DBMS_OUTPUT.PUT_LINE('Estado: '||l_estado|| ' / viaje_id:  '||r_viajes.viaje_id); 
      END IF;

           IF l_estado = 'EN_DESCARGA' THEN

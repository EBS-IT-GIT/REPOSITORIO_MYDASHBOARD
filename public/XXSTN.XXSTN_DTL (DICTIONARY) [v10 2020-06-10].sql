...
----------------------------------------------------------------------------------------------------
	...
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_ESTABLISHMENT.location_id IS 'Location ID of the Registered Address. Foreign key to LOCATION (HR_LOCATIONS_ALL).';
...
----------------------------------------------------------------------------------------------------
COMMENT ON TABLE XXSTN.XXSTN_DTL_MOVEMENT_BATCH IS 'MOVEMENT_BATCH (GL_JE_BATCHES) stores journal entry batches. Each row includes the batch name, description, status, running total debits and credits, and other information. This table corresponds to the Batch window of the Enter Journals form. STATUS is ''U'' for unposted, ''P'' for posted, ''S'' for selected, ''I'' for in the process of being posted. Other values of status indicate an error condition. STATUS_VERIFIED is ''N'' when you create or modify an unposted journal entry batch. The posting program changes STATUS_VERIFIED to ''I'' when posting is in process and ''Y'' after posting is complete.';
----------------------------------------------------------------------------------------------------
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.batch_id IS 'Journal entry batch identifier (JE_BATCH_ID)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.chart_of_accounts_id IS 'Chart of Accounts (ID_FLEX_NUM)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.date_created IS 'Date batch was created (dd-MMM-yy hh:mm:ss)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.posted_date IS 'Date batch was posted (dd-MMM-yy hh:mm:ss)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.default_period_name IS 'Accounting period for batch (MMM-yy)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.default_effective_date IS 'Date within default accounting period (dd-MMM-yy)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.earliest_postable_date IS 'Earliest date batch can be posted (dd-MMM-yy hh:mm:ss)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.accounted_period_type IS 'Accounting period type (Month, Quarter, Year, Week or Custom)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.period_calendar IS 'Accounting calendar name (PERIOD_SET_NAME)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.status IS 'Journal entry batch status ''U'' for unposted, ''P'' for posted, ''S'' for selected, ''I'' for in the process of being posted';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.status_verified IS 'Batch status verified by posting process ''I'' when in process and ''Y'' after complete';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.balance_type IS 'Balance type (ACTUAL_FLAG) ''A'' is Actual, ''B'' is Budget, or ''E'' is Encumbrance';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.batch_name IS 'Name of journal entry batch';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.batch_description IS 'Journal entry batch description';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.is_average_journal IS 'Average journal flag ''N'' is Standard, ''Y'' is Average';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.budgetary_control_status IS 'Journal entry batch funds check status ''F'' is Failed, ''I'', In Process, ''N'', N/A, ''P'', Passed, ''R'', Required';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.parent_batch_id IS 'Defining column of the parent batch in the source reporting currency ledger (PARENT_JE_BATCH_ID)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.approval_status_code IS 'Journal entry batch approval status ''A'' is Approved, ''I'', In Process, ''J'', Rejected, ''R'', Required, ''V'',Validation Failed,''Z'', N/A';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.posting_run_id IS 'Posting sequence number';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.request_id IS 'Posting concurrent request id';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.org_id IS 'Organization defining column';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.posted_by IS 'User who posted the journal batch';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.group_id IS 'Interface group identifying column';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.approver_employee_id IS 'Defining column of the employee who submitted the journal batch for approval';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.last_update_date IS 'Indicates the date and time of the last update of the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.last_updated_by IS 'Indicates the user who last updated the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.creation_date IS 'Indicates the date and time of the creation of the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.created_by IS 'Indicates the user who created the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_BATCH.last_update_login IS 'Indicates the session login associated to the user who last updated the row';
	

----------------------------------------------------------------------------------------------------
COMMENT ON TABLE XXSTN.XXSTN_DTL_MOVEMENT_HEADER IS 'MOVEMENT_HEADER (GL_JE_HEADERS) stores journal entries. There is a one-to-many relationship between journal entry batches and journal entries. Each row in this table includes the associated batch ID, the journal entry name and description, and other information about the journal entry. This table corresponds to the Journals window of the Enter Journals form. STATUS is ''U'' for unposted and ''P'' for posted. Other statuses indicate that an error condition was found. CONVERSION_FLAG equal to ''N'' indicates that you manually changed a converted amount in the Journal Entry Lines zone of a foreign currency journal entry. In this case, the posting program does not re-convert your foreign amounts. Balancing Company (BALANCING_SEGMENT_VALUE) is null if there is only one balancing segment value in your journal entry.';
----------------------------------------------------------------------------------------------------
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.header_id IS 'Journal entry header identifier (JE_HEADER_ID)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.accounting_book_id IS 'Accounting Book (LEDGER_ID)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.batch_id IS 'Journal entry batch identifier (JE_BATCH_ID)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.period_name IS 'Accounting period (MMM-yy)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.status IS 'Journal entry header status ''U'' for unposted and ''P'' for posted. Other statuses indicate that an error condition';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.currency_code IS 'Currency code (ISO 4217 three-letter)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.date_created IS 'Date header created in GL (dd-MMM-yy hh:mm:ss)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.posted_date IS 'Date journal entry header was posted (dd-MMM-yy hh:mm:ss)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.default_effective_date IS 'Journal entry effective date (dd-MMM-yy)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.earliest_postable_date IS 'Earliest date journal entry header can be posted (dd-MMM-yy hh:mm:ss)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.category IS 'Journal entry category (Payroll, Intercompany Transfer, Depreciation, Tax, Payments, Purchase Invoices, Adjustment, Reconciled Payments, etc.)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.source IS 'Journal entry source (Manual, Payables, Spreadsheet, Receivables, Consolidation, Assets etc.)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.header_name IS 'Journal entry header name';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.header_description IS 'Journal entry description';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_accrual_rev IS 'Reversed journal entry flag';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_multi_bal_seg IS 'Multiple balancing segment flag';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.balance_type IS 'Balance type (ACTUAL_FLAG) ''A'' is Actual, ''B'' is Budget, or ''E'' is Encumbrance';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.tax_status_code IS 'Journal entry tax status ''N'' is Not Required, ''R'' is Required, ''T'' is Taxed''';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_conversion IS 'Currency conversion flag';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_balanced IS 'Balanced journal entry flag';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.balancing_company IS 'Key flexfield structure balancing segment value';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.accrual_rev_period_name IS 'Reversed journal entry reversal period (MMM-yy)';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.accrual_rev_status IS 'Reversed journal entry status ''M'' is Marked for Reversal, ''N'' is Not Reversed, ''R'' is Reversed, ''U'' is Unreversable';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.accrual_rev_header_id IS 'Reversed journal entry defining column';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_accrual_rev_change_sign IS 'Type of reversal ''Y'' for Change Sign or ''N'' for Switch DR/CR';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.parent_header_id IS 'Defining column of the parent journal entry in the source primary ledger';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.reversed_header_id IS 'Defining column of the journal entry that is reversed by this journal entry';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.is_from_sla IS 'Journal propagated from subledger flag';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.last_update_date IS 'Indicates the date and time of the last update of the row';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.last_updated_by IS 'Indicates the user who last updated the row';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.creation_date IS 'Indicates the date and time of the creation of the row';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.created_by IS 'Indicates the user who created the row';
    COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_HEADER.last_update_login IS 'Indicates the session login associated to the user who last updated the row';
		

----------------------------------------------------------------------------------------------------
COMMENT ON TABLE XXSTN.XXSTN_DTL_MOVEMENT_LINES IS 'MOVEMENT_LINES (GL_JE_LINES) stores the journal entry lines that you enter in the Enter Journals form. There is a one-to-many relationship between journal entries and journal entry lines. Each row in this table stores the associated journal entry header ID, the line number, the associated Accounting Reference (CODE_COMBINATION_ID), and the debits or credits associated with the journal line. STATUS is ''U'' for unposted or ''P'' for posted.';
----------------------------------------------------------------------------------------------------
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.line_id IS 'Journal entry identifier (JE_HEADER_ID + JE_LINE_NUM)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.header_id IS 'Journal entry header reference (JE_HEADER_ID)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.line_num IS 'Journal entry line number (JE_LINE_NUM)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.accounting_book_id IS 'Accounting Book (LEDGER_ID)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.accounting_reference_id IS 'Key combination of Segments (Company, Brand, Product, Account Unit, Cost Center, Intercompany, Business Unit, Future) or other combination, depending on Chart of Accounts setup (CODE_COMBINATION_ID)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.period_name IS 'Accounting period name (MMM-yy)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.effective_date IS 'Journal entry line effective date (dd-MMM-yy)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.status IS 'Journal entry line status ''U'' for unposted or ''P'' for posted.';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.entered_dr IS 'Journal entry line debit amount in entered currency';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.entered_cr IS 'Journal entry line credit amount in entered currency';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.accounted_dr IS 'Journal entry line debit amount in base currency';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.accounted_cr IS 'Journal entry line credit amount in base currency';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.movement_description IS 'Journal entry line description';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.line_type_code IS 'Line type (Suspense, Rounding, Inter)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_1 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_2 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_5 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_3 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_6 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_4 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_7 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_8 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_9 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.reference_10 IS 'Journal entry external reference (depends on entry module AP, AR, PO...)';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.context2 IS 'Descriptive flexfield context column';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.no1 IS 'Value added tax descriptive flexfield column';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.ignore_rate IS 'Modify amounts if exchange rate changes';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.last_update_date IS 'Indicates the date and time of the last update of the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.last_updated_by IS 'Indicates the user who last updated the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.creation_date IS 'Indicates the date and time of the creation of the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.created_by IS 'Indicates the user who created the row';
	COMMENT ON COLUMN XXSTN.XXSTN_DTL_MOVEMENT_LINES.last_update_login IS 'Indicates the session login associated to the user who last updated the row';
